...
int minimax(struct position currentposition,int depth,int alpha,int beta,int maximizingplayer)
{
    if(depth == 0)
    {
...
int findbestmove(struct position board)
{
    int bestmove = 0;
    int bestscore = -99999;
    int score;
    for(int i = 0;i<=6;i++)
    {
        if(isvalidmove(i,board) == 0)
                continue;
        struct position newboard = domove(i,board,'r');
        score = minimax(newboard,7,-99999,99999,0);
        //printf("   %d   ",score);
        if(score > bestscore)
        {
            bestscore = score;
...
int main()
{
    ...
    while(1)
    {
        ...
        printboard(board);
        printf("%d ",findbestmove(board));
        printf("\n");
        board = domove(findbestmove(board),board,'r');
        printboard(board);
    }
    return 0;
}
